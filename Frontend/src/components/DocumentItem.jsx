...
function DocumentItem(props) {
  const {document,viewType,onSelect} = props;
  ...
  return (
    <div className={`${viewType!=='list'?'lg:w-1/5 md:w-1/2 p-2':'w-full p-1'} `} onClick={() => onSelect(document)}>
      <div className={`${viewType!=='list'?'shadow-md':'flex items-center p-1'} border-2 border-gray-200 bg-white rounded-lg  hover:shadow-lg hover:border-blue-500 transition duration-300 group`}>
        <div className={`${viewType!=='list'?'block relative h-32 rounded-t-lg overflow-hidden':''} `}>
          {/* Badge */}
          <span className={`${viewType!=='list'?'absolute top-2 right-2 bg-blue-50 text-blue-400 text-xs font-semibold px-2 py-1 rounded-full shadow-md z-10':'hidden'} `}>
          ...
          /> */}
            ...
            <div  className={` ${viewType==='list'?'hidden':''} `}>
              <DocumentPreview docpath={document.DocumentPath}/>
            </div>
          <i className={` ${viewType!=='list'?'hidden':'fas fa-file text-blue-500 text-xl p-1'} `}></i>
        </div>
